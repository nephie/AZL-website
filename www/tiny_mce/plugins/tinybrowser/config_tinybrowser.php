...
// OWN HACK --> om de ntlm auth te omzeilen zetten we deze vars zodat de upload niet via aan ntlm beveiligde url gedaan wordt
$uploadpathhack = 'http://intranet.azlokeren.be/tiny_mce/plugins/tinybrowser';
if(!isset($_GET['normalpath'])){

	$ref = $_SERVER['HTTP_REFERER'];
	$pieces = parse_url($ref);
	parse_str($pieces['query'],$vars);

	if(isset($vars['normalpath'])){
		$normalpathhack = $vars['normalpath'];
	}
	else {
		$normalpathhack = (($_SERVER['HTTPS'] == 'off' || $_SERVER['HTTPS'] == '')? 'http' : 'https') . '://' . $_SERVER['SERVER_NAME'] . '/tiny_mce/plugins/tinybrowser';
	}

}
else {
	$normalpathhack = $_GET['normalpath'];
}
...
$tinybrowser['obfuscate'] = 's0merand0mfgbhfdjunk!!!111';

// Set default language (ISO 639-1 code)
$tinybrowser['language'] = 'NL';
...
$tinybrowser['path']['image'] = '/files/useruploads/images/'; // Image files location - also creates a '_thumbs' subdirectory within this path to hold the image thumbnails
$tinybrowser['path']['media'] = '/files/useruploads/media/'; // Media files location
$tinybrowser['path']['file']  = '/files/useruploads/files/'; // Other files location
