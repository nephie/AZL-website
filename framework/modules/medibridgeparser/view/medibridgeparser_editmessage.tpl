<h1>Aanpassen bericht: {$log->getFilename()}</h1>
...
<table>
	...
	{/if}
	{if $log->getStatusdelivery() == "DELIVERY_ERROR" || $log->getStatusdelivery() == "PARSER_ERROR" || $log->getStatusdelivery() == "NO_PARSER"}
	<tr>
		<td>
...
<p>
	<a href="#" onClick="{ajaxrequest request=$ignore}">Negeren</a>
</p>
<p>
&nbsp;
</p>
Bron van het bestand: {$sourcelocation}
<p>
</p>
{include file="form.tpl" form=$form}
<p>
	Na het doorsturen van dit formulier zal het bestand verzet worden naar "{$destination}", waarna het terug verwerkt zal worden.
</p>
<p>&nbsp;</p>